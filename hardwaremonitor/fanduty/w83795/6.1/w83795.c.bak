...
static struct w83795_data *w83795_update_device(struct device *dev)
{
	 ...
	 * It is important to read intrusion first, because reading from the
	 * SMI_STS6 register apparently clears the interrupt status
	 * temporarily. */
	tmp = w83795_read(client, W83795_REG_ALARM_CTRL);
	/* Switch to interrupt status for intrusion if needed */
	if (tmp & 0x80)
		w83795_write(client, W83795_REG_ALARM_CTRL, tmp & ~0x80);
 ...
 * enabled.
 */
static int w83795_tss_useful(const struct w83795_data *data, int ts)
{
	int useful = 0;
	int i;

	for (i = 0; i < 4; i++) {
		if (tss_map[i][ts] == TSS_MAP_RESERVED)
			continue;
		if (tss_map[i][ts] < 6)		/* Analog */
			useful += (data->has_temp >> tss_map[i][ts]) & 1;
		else				/* Digital */
			useful += (data->has_dts >> (tss_map[i][ts] - 6)) & 1;
	}
...
show_temp_src(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (tmp >= 4 || tss_map[tmp][index] == TSS_MAP_RESERVED)
		return -EINVAL;	/* Shouldn't happen, but...) */
...
#else
static struct sensor_device_attribute_2 w83795_temp[][28] = {
#endif
	SENSOR_ATTR_TEMP(1),
	SENSOR_ATTR_TEMP(2),
	SENSOR_ATTR_TEMP(3),
...
#else
static struct sensor_device_attribute_2 w83795_pwm[][8] = {
#endif
	SENSOR_ATTR_PWM(1),
	SENSOR_ATTR_PWM(2),
	SENSOR_ATTR_PWM(3),
...
#else
static struct sensor_device_attribute_2 w83795_tss[6] = {
#endif
...
#endif
{
	...
	for (i = 0; i < ARRAY_SIZE(w83795_tss); i++) {
		...
		if (!j)
			continue;
		dev_dbg(dev, "%d temperature channel%s usable for source %d\n",
			j, j < 2 ? "" : "s", i + 1);
