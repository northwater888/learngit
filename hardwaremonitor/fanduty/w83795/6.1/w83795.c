...
int hwm_major=0;
int hwm_minor=0;
struct hwm_dev *hwm_dev=NULL;

struct device *golbaldev=NULL;
struct i2c_client *globalclient = NULL;
...
//printk("==>w83795_detect\n");

	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA))
		return -ENODEV;
	//printk("bank:%x\n",i2c_smbus_read_byte_data(client, w83795_REG_BANKSEL));
...
//printk("vendor_id:%x\n",i2c_smbus_read_byte_data(client, w83795_REG_VENDORID));
...
//printk("device_id:%x\n",i2c_smbus_read_byte_data(client, w83795_REG_CHIPID));
			...
			const struct i2c_device_id *id)
{
...
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2, 6, 26)
	/* Switch to our copy of the client */
	memcpy(&data->client, client, sizeof(struct i2c_client));
	client = &data->client;
	dev = &client->dev;
  ...
  globalclient = client;
  //printk("<==w83795_probe, globalclient = %x, client = %x\n",globalclient, client);
	return 0;
...
static int w83795_detect_legacy(struct i2c_adapter *adapter, int address,
				int kind)
{
	struct i2c_client client;
	struct i2c_board_info info;
	...
	/* Setup fake client for probing */
	memset(&client, 0, sizeof(struct i2c_client));
	client.adapter = adapter;
	client.addr = address;
	client.driver = &w83795_driver;

	/* Detect device */
	memset(&info, 0, sizeof(struct i2c_board_info));
	err = w83795_detect(&client, kind, &info);
	if (err)
		return 0;
	...
	did = w83795_id;
	while (did->name) {
		if (strcmp(did->name, info.type) == 0)
			break;
		did++;
	}
	if (!did->name)
	...
	strlcpy(client.name, info.type, I2C_NAME_SIZE);
	return w83795_probe(&client, did);
}

static int w83795_attach_adapter(struct i2c_adapter *adapter)
{
	if (!(adapter->class & I2C_CLASS_HWMON))
		return 0;
	return i2c_probe(adapter, &addr_data, w83795_detect_legacy);
}
#endif
...
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2, 6, 26)
	.attach_adapter	= w83795_attach_adapter,
	.detach_client	= w83795_remove,
#else
	.probe		= w83795_probe,
...
{
	struct hwm_dev *hwm_dev;
	hwm_dev=container_of(inode->i_cdev,struct hwm_dev,hwm_set_dev);
	filp->private_data=hwm_dev;
	
	if((filp->f_flags&O_ACCMODE) == O_WRONLY)
	{
		if(down_interruptible(&hwm_dev->hwmsem))
			return -ERESTARTSYS;
		memset(&hwm_dev->hwm_data,0,sizeof(int));
		up(&hwm_dev->hwmsem);
	}
	return 0;
}
...
int get_value(struct i2c_client *client,u8 arg)
{
   u8 data;
   data=w83795_read(client, arg);
  // printk("pos is:%x,datais:%x\n",arg,data);
  return data;
}
 int set_value(struct i2c_client *client,unsigned long arg)
{
  u8 pos ,data,ret ;
  pos=arg>>24|0;
  data=arg&0xff;
 // printk("posis:%x,datais:%x\n",pos,data);
  //data=w83795_read(client, pos);
 // printk("pos is:%x,datais:%x\n",pos,data);

  ret=w83795_write(client, pos, data);
	return ret;
}
...
{
	int err=0,ret=0;
  struct i2c_client *client =globalclient;
	
	if(_IOC_TYPE(cmd)!=HWM_IOC_MAGIC) 
		return -ENOTTY;
	if(_IOC_NR(cmd)>HWM_IOC_MAXNR)
		return -ENOTTY;
	...
	{
		case HWM_I0C_QUERY:
		  ret =  get_value(client, arg);
		  __put_user(ret, (int __user *)arg);	
		  break;
		case HWM_I0C_SET:
			ret =  set_value(client, arg);
		  break;
    default:
			return -ENOTTY;
	}
...
static int __init sensors_w83795_init(void)
{
  ...
  int result,err;
	dev_t devno=0;
	 ...
	 i2c_add_driver(&w83795_driver);
	
	if(hwm_major!=0)
	{
		devno=MKDEV(hwm_major,hwm_minor);
		result=register_chrdev_region(devno,1,"hwm");
	}
	else
	{
		result=alloc_chrdev_region(&devno,hwm_minor,1,"hwm");
		hwm_major=MAJOR(devno);
	}
	printk("hwmmajor: %d\n hwmminor: %d\n",hwm_major,hwm_minor);

	if(result<0) 
	{
		printk(KERN_WARNING"hwm can't get major:%d",hwm_major);
		return result;
	}
	
	hwm_dev=(struct hwm_dev *)kmalloc(sizeof(struct hwm_dev),GFP_KERNEL);
	if(hwm_dev==NULL)
	{
		result=-ENOMEM;
		goto fail;
	}
	memset(hwm_dev,0,sizeof(struct hwm_dev));
		
	init_MUTEX(&hwm_dev->hwmsem);
	devno=MKDEV(hwm_major,hwm_minor);
	cdev_init(&hwm_dev->hwm_set_dev,&fops);
	hwm_dev->hwm_set_dev.owner=THIS_MODULE;
	hwm_dev->hwm_set_dev.ops=&fops;
	
	err=cdev_add(&hwm_dev->hwm_set_dev,devno,1);
	if(err)
	{
		printk(KERN_NOTICE"Error %d when add hwmdev",err);
		return -1;
	}
