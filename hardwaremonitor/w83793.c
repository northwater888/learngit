...
int get_value(struct i2c_client *client,u8 arg)
{
  u8 res;
  //int i;
  res= w83793_read_value(client, arg);
    //printk("get  %x get data:%d  ",arg,res);
//  for (i = 0x23; i < 0x32; i++)
//  { 
//     res= w83793_read_value(client, i);
//     printk("  %x data:%d  ",i,res);
//     if(i%4==0)
//       printk("\n");
//  }

  //i2c_smbus_write_byte_data(client, W83793_REG_BANKSEL, reg) ;
//  res = i2c_smbus_read_byte_data(client, W83793_REG_DEVICEID);
//  printk("W83793_REG_DEVICEID:%x\n",res);
//  res = i2c_smbus_read_byte_data(client, W83793_REG_VENDORID);
//  printk("W83793_REG_VENDORID:%x\n",res);
//  
//  res = i2c_smbus_read_byte_data(client, res);
//  printk("W83793_REG_DEVICEID:%x\n",res);
//  res = i2c_smbus_read_byte_data(client, res);
//  printk("W83793_REG_DEVICEID:%x\n",res);
//  
...
{
	int err=0,ret=0;
        printk("==>hwm_ioctl dev = %x\n",(int)golbaldev);
	struct i2c_client *client = to_i2c_client(golbaldev);
	struct w83793_data *data;
	data=w83793_update_device(golbaldev);
...
//  //dev_t devno=MKDEV(hwm_major,hwm_minor);
//	
//	if(hwm_dev)
//	{
//		memset(hwm_dev,0,sizeof(hwm_dev));
//		kfree(hwm_dev);
//		cdev_del(&hwm_dev->hwm_set_dev);
//	}
//	
